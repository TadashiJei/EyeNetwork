...
GATEWAY=$(cat /opt/eyenet/gateway)
SERVERKEY=$(cat /opt/eyenet/serverkey)
...
function extractMdadm() {
    if command -v mdadm &> /dev/null
    then
        devices="$(awk '/^md/ {print  $1}' /proc/mdstat)"
        for device in $devices
        do
            echo $device
            echo ":"
            echo $(sudo mdadm --detail /dev/$device | grep 'State :' | awk -F\: '{print $2}')
            echo ";"
        done
    fi
}
